...
class database{
		...
		if(count($where) > 0){
			...
			foreach($where as $field_name => $field_value){
				if($field_value == 'null'){
					...
					if(in_array(substr($field_value, 0, 1), array('>', '<'))){
			...
			for($j=1;$j<$i;$j++){
				if(in_array(substr($filter[$j], 0, 2), array('OR'))){
	...
	//==============================================================================
	public function createSQL($table, $fields, $where, $sort, $group, $type){
	...
	//==============================================================================
	public function request($table, $fields, $where, $sort = null, $group = null){
		//if the connection is alive, execute the query
		if($this->conexao){
			//create the SELECT statement
			$query = $this->createSQL($table, $fields, $where, $sort, $group, "SELECT");
	...
	public function add($table, $values){
		...
		if($this->conexao){
			...
			$query = $this->createSQL($table, $values, null, null, null, "INSERT");
			//execute the query in the database
			$result = @mysql_query($query);
			//if the result is populated, format the resultset and encode it as JSON for output
			if($result){
				$fields[0] = '*';
				$query = $this->createSQL($table, $fields, $values, null, null, "SELECT");
				$result = @mysql_query($query);
				if($result){
					$num = mysql_affected_rows();
	...
	public function update($table, $values, $where){
		...
		if($this->conexao){
			...
			$query = $this->createSQL($table, $values, $where, null, null, "UPDATE");
			//execute the query in the database
			$result = @mysql_query($query);
			//if the result is populated, format the resultset and encode it as JSON for output
			if($result){
	...
	public function remove($table, $where){
		...
		if($this->conexao){
			...
			$query = $this->createSQL($table, null, $where, null, null, "DELETE");
			//execute the query in the database
			$result = @mysql_query($query);
			//if the result is populated, format the resultset and encode it as JSON for output
			if($result){
